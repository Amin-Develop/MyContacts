    ...
    public partial class AddContact : Window
    {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            if (_vl.IsContactValid(txtFullName.Text, txtIp.Text, int.Parse(txtAge.Text)))
            {
                if (imgDialog != null)
                {
                    Contact contact = new Contact() { Age = int.Parse(txtAge.Text), FullName = txtFullName.Text, Ip = txtIp.Text, User = _user };
                ...
                {
                    txtRes.Text = "لطفا عکس را انتخاب کنید";
                }
            }
            else
            {
                txtRes.Text = _vl.ReturnError();
            }
        }
